package it.maggioli.eldasoft.bl.arxivar;

import it.abletech.arxivar.dati.ARXDati;
import it.abletech.arxivar.dati.ARXDatiSoap;
import it.abletech.arxivar.documenti.ARXDocumenti;
import it.abletech.arxivar.documenti.ARXDocumentiSoap;
import it.abletech.arxivar.login.ARXLogin;
import it.abletech.arxivar.login.ARXLoginSoap;
...
public class ArxivarManager implements IWSDMManager {

  static Logger         logger               = Logger.getLogger(ArxivarManager.class);

  static private String ARXIVAR_SOFTWARENAME = "java:comp/env/ARXIVAR_SOFTWARENAME";
  static private String ARXIVAR_WSLOGIN      = "java:comp/env/ARXIVAR_WSLOGIN";
  static private String ARXIVAR_WSSEARCH     = "java:comp/env/ARXIVAR_WSSEARCH";
  static private String ARXIVAR_WSDATI       = "java:comp/env/ARXIVAR_WSDATI";
  static private String ARXIVAR_WSDOCUMENTI  = "java:comp/env/ARXIVAR_WSDOCUMENTI";

  private ARXLoginSoap getArxLoginSoap() throws Exception {
    String wsLoginAddress = InitialContext.doLookup(ARXIVAR_WSLOGIN);
    if (wsLoginAddress == null || (wsLoginAddress != null && "".equals(wsLoginAddress.trim()))) {
      throw new Exception(URL_NOT_DEFINED);
    }
    ARXLogin arxLogin = new ARXLogin();
    ARXLoginSoap arxLoginSoap = arxLogin.getARXLoginSoap();
    Client client = ClientProxy.getClient(arxLoginSoap);
    client.getRequestContext().put(Message.ENDPOINT_ADDRESS, wsLoginAddress);
    return arxLoginSoap;
  }

  private ARXDatiSoap getArxDatiSoap() throws Exception {
    String wsDatiAddress = InitialContext.doLookup(ARXIVAR_WSDATI);
    if (wsDatiAddress == null || (wsDatiAddress != null && "".equals(wsDatiAddress.trim()))) {
      throw new Exception(URL_NOT_DEFINED);
    }
    ARXDati arxDati = new ARXDati();
    ARXDatiSoap arxDatiSoap = arxDati.getARXDatiSoap();
    Client client = ClientProxy.getClient(arxDatiSoap);
    client.getRequestContext().put(Message.ENDPOINT_ADDRESS, wsDatiAddress);
    return arxDatiSoap;
  }

  private ARXDocumentiSoap getArxDocumentiSoap() throws Exception {
    String wsDocumentiAddress = InitialContext.doLookup(ARXIVAR_WSDOCUMENTI);
    if (wsDocumentiAddress == null || (wsDocumentiAddress != null && "".equals(wsDocumentiAddress.trim()))) {
      throw new Exception(URL_NOT_DEFINED);
    }
    ARXDocumenti arxDocumenti = new ARXDocumenti();
    ARXDocumentiSoap arxDocumentiSoap = arxDocumenti.getARXDocumentiSoap();
    Client client = ClientProxy.getClient(arxDocumentiSoap);
    client.getRequestContext().put(Message.ENDPOINT_ADDRESS, wsDocumentiAddress);
    return arxDocumentiSoap;
  }
  ...
  public WSDMProtocolloDocumentoRes _protocolloLeggi(String username, String password, WSDMLoginAttr loginAttr, Long annoProtocollo,
    ...
    // ARXLoginSoap arxLoginSoap = null;
    //
    // try {
    // arxLoginSoap = this.getArxLoginSoap();
    // String softwareName = InitialContext.doLookup(ARXIVAR_SOFTWARENAME);
    // LoginModel loginModel = arxLoginSoap.login(username, password,
    // softwareName);
    // isLoggedIn = loginModel.isLoggedIn();
    // if (isLoggedIn) {
    // sessionId = loginModel.getSessionId();
    //
    // ARXDatiSoap arxDatiSoap = this.getArxDatiSoap();
    // DmProfile dmProfile = arxDatiSoap.dmProfileGetDataByDocNumber(sessionId,
    // new Long(numeroProtocollo).intValue());
    //
    // DmProfileAllegati dmProfileAllegati = dmProfile.getALLEGATI();
    //
    // dmProfile.getINOUT();
    // dmProfile.getDESTINATARIO();
    // dmProfile.getMITTENTE();
    //
    // wsdmprotocolloDocumentoRes.setEsito(true);
    // wsdmprotocolloDocumentoRes.setMessaggio("Connesso con sessionId: " +
    // sessionId);
    //
    // } else {
    // wsdmprotocolloDocumentoRes.setEsito(false);
    // wsdmprotocolloDocumentoRes.setMessaggio(loginModel.getArxLogOnErrorTypeString());
    // }
    //
